...
* 调用int指令，跳转到IDT中，选择中断处理服务并执行。因为从用户态转换到内核态时，需要满足IDT表项中的DPL 大于等于 CPL，所以将其特殊处理一下。

* 之后在trap_dispatch函数中，将保存的trapframe中的段寄存器、栈基址均进行修改，最后iret时就可以完成状态的转换。

* 从内核态调用int指令时，没有保存%esp, %ss寄存器，所以需要将%esp减8留出空间；从用户态调用int指令后也类似，需要将%esp加8，忽略掉最后的%esp和%ss寄存器。
